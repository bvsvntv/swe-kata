...
const fetchAddresses = `-- name: FetchAddresses :many
SELECT address_id, address, address2, district, city_id, postal_code, phone, last_update FROM address
ORDER BY address_id
LIMIT $1 OFFSET $2
`

type FetchAddressesParams struct {
	Limit  int32 `json:"limit"`
	Offset int32 `json:"offset"`
}

func (q *Queries) FetchAddresses(ctx context.Context, arg FetchAddressesParams) ([]Address, error) {
	rows, err := q.db.Query(ctx, fetchAddresses, arg.Limit, arg.Offset)
	if err != nil {
		return nil, err
	}
	defer rows.Close()
...
const updateAddressPartial = `-- name: UpdateAddressPartial :one
UPDATE address
SET
...
type UpdateAddressPartialParams struct {
	Address    pgtype.Text `json:"address"`
	Address2   pgtype.Text `json:"address2"`
	District   pgtype.Text `json:"district"`
	CityID     pgtype.Int2 `json:"city_id"`
	PostalCode pgtype.Text `json:"postal_code"`
	Phone      pgtype.Text `json:"phone"`
	AddressID  int32       `json:"address_id"`
}

func (q *Queries) UpdateAddressPartial(ctx context.Context, arg UpdateAddressPartialParams) (Address, error) {
	row := q.db.QueryRow(ctx, updateAddressPartial,
		arg.Address,
		arg.Address2,
		arg.District,
		arg.CityID,
